...
import heroImg from "./assets/pics/13.jpeg";
import { Info } from "lucide-react";
import { FaPlay } from "react-icons/fa6";
import { useEffect, useState } from "react";
import Modal from "./components/Modal";
import ModalPerMonth from "./components/ModalPerMonth";
...
function App() {
  ...
  const [monthPicked, setMonthPicked] = useState('January');
        ...
        <div className="py-5 lg:py-10 flex flex-col gap-5 px-5 lg:px-10 max-w-screen-2xl">
          ...
          <section className=" grid grid-cols-2 lg:grid-cols-6 gap-3 lg:gap-5">
            {images.map((image, idx) => (
              <CardImage src={image.src} key={idx} idx={idx} month={image.month} onClick={() => { 
                setOpenModalMonth(true)
                setMonthPicked(image.month)
              }}/>
            ))}
          </section>
        </div>
      </section>
      <ModalPerMonth setOpenModal={setOpenModalMonth} openModal={openModalMonth} images={monthPicked} ></ModalPerMonth>
